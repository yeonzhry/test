    ...
    if (!isMicActive) {
        ...
        navigator.mediaDevices.getUserMedia({ audio: true })
            .then(stream => {
                ...
                isPlaying = true;
                isMicActive = true; // 마이크 활성화 상태 설정
                micButton.textContent = '마이크 끄기'; // 버튼 텍스트 변경
                drawVisualizer();
            })
            .catch(error => {
        ...
        // AudioContext 종료 (필요 시)
        if (audioContext) {
            audioContext.close();
            audioContext = null;
        }

        isMicActive = false; // 마이크 비활성화 상태 설정
        micButton.textContent = '마이크 켜기'; // 버튼 텍스트 변경
